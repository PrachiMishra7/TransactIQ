...
db = SessionLocal()
try:
    tab1, tab2, tab3 = st.tabs(["Validation Rules", "Payment Allow-List", "Date Formats"])
    ...
    with tab2:
        ...
        st.markdown('<p style="color:#475569; font-size:0.82rem; margin-top:16px;">Any value outside this list will be automatically flagged as an invalid payment method during processing.</p>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with tab3:
        ...
        st.markdown('<p style="color:#475569; font-size:0.82rem; margin-top:16px;">The engine automatically parses and validates these datetime string formats. Invalid dates or malformed strings will fail validation.</p>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
